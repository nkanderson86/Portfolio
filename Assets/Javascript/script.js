...
let projectsArr = [
  {
    ...
    description:
      "First team project, focused on solving the problem of what to eat and where to eat it.  Our team used two different APIs (Edamam & Yelp) to retreive and display results based on user's input"
  },
  {
    name: "Elevate",
    ...
    description:
      "Second team project, focused on helping people find a drink or strain to their liking.  Our team built a full stack project with a backend that stores and authenticates users visitng the site."
  },
  {
...
function buildCards() {
  ...
  for (let i = 0; i < projectsArr.length; i++) {
    ...
    let projectName = $("<h4>")
      .text(projectsArr[i].name)
      .addClass("card-title p-2");
    let imageDiv = $("<div>")
      .addClass("card m-2 p-1 col-md-3")
      .attr("style", "background-color:  #1d2731;");
    let projectImg = $("<img>")
      .attr("src", projectsArr[i].imageSrc)
      .attr("style", "width:100%;height:auto;overflow:auto;")
      .addClass("card-top-img");
    let description = $("<p>")
      .addClass("description")
      .attr("style", "color:white;")
      .text(projectsArr[i].description);
    let repoLink = $("<a>")
      .attr("href", projectsArr[i].git)
      .attr("target", "_blank")
      .text("Check out the code!");
    let liveLink = $("<a>")
      .attr("href", projectsArr[i].link)
      .attr("target", "_blank")
